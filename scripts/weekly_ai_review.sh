...
2c. OPEN HYPOTHESES pending out-of-sample validation from ~2026-09-08 onward — check auto-memory for the full detail, and if enough NEW trades have accumulated since 2026-08-19, re-test these on the fresh slice rather than re-mining the old sample: (a) ENTRY side — excluding CFO-titled insider signals / tilting toward Director-titled ones showed an alpha lift across three cuts of a 9-month EDGAR replay; (b) EXIT side — the live flat 12% take-profit is unreachable for 58% of positions (they can only ever exit at the stop) and was the WORST of nine exit methods tested on alpha; resistance-zone exits (prior 60d high) lead on win-rate. Neither is implemented. Both were discovered by mining a 41-trade sample, and a sensitivity check showed the volatility-scaled variant swings 8x on an arbitrary lookback choice — so require robustness (insensitivity to arbitrary window/parameter choices) before recommending either, and say so explicitly if the fresh sample is still too small to conclude.
