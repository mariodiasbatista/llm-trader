...
2d-2. QUEUED EXIT HYPOTHESIS (not implemented): a hard timeout — 'after ~22 days holding, take any profit that exists' — tested +\$546 P&L / +0.29pp alpha over the live config, with a smooth plateau across 18-25 days that lines up with the day-20 median time-to-peak. BUT only 7 of 42 positions were affected and ONE of them (KEYS, rescued from a day-37 stop-out) accounts for +\$972 of the +\$546 net; strip it and the rule is negative. Gradual/linear target decay was tested separately and does NOT work — median holding is only 10 days so a day-20+ decay never binds, and earlier-biting variants actively lose money. If re-testing: use only trades entered after 2026-08-19, and the decisive check is whether the timeout still helps with the single biggest contributor EXCLUDED. Defer again if it still touches fewer than ~10 positions.
...
2g. WHERE TO FOCUS (established 2026-09-03): the EXIT side is done — the per-stock adaptive take-profit and 15% stop are working, and further exit tuning has low expected value. The binding constraint is now the ENTRY/PRE-CHECK layer, where measurement shows filters are REMOVING profitable opportunity: replaying every decision (SKIPs reconstructed from bot.log + buys from trades.log) under the live exit config gives AI-selected buys +1.02% alpha, the signals the AI SKIPPED +1.67% alpha, and buying every SEC signal with no filter at all +1.34% — i.e. the AI gate and the \$50 price floor each subtract alpha, and the live combination (AI + \$50 floor, +0.75%) is close to the worst tested. Every AI skip category was wrong, worst of all its 'sector/company judgment' skips. ALSO: the \$50 floor cannot be validated from data — all 31 sub-\$50 signals predate it and zero exist after, so the filter suppresses the evidence needed to judge it (a filter that removes data always looks defensible on the data that remains). Do NOT propose further exit-parameter tuning as the route to more profit; direct any strategy work at the entry gate. Full numbers and the honest caveats (single 3-month window, contradicts the July min_entry_price finding which predated alpha tooling) are in auto-memory.
2f. TRADES.LOG WAS CORRECTED ON 2026-09-01 — before that date it recorded the current market price instead of the real broker fill whenever an order was not confirmed within a 15-second wait, and it contained one entirely phantom trade (HWKN, an order that EXPIRED unfilled at the bell but was journaled as a completed TAKE_PROFIT while the position was silently deleted from state.json, leaving real stock untracked). 8 sell/buy prices have been corrected to actual broker fills and tagged 'price_corrected_from=... real_fill_from_broker=true'; the phantom is neutralised as action 'VOID_PHANTOM_TAKE_PROFIT' (matches no parser — do NOT count it). Realized P&L was overstated by roughly \$2,500, so ANY dollar figure quoted from analysis done before 2026-09-01 — including the adaptive-take-profit expectancy and the exit-method comparison tables in auto-memory — is optimistic and must be recomputed rather than cited. When auditing P&L, cross-check trades.log against broker order history; this is the second time a journal defect produced false conclusions (the first was WHEEL losses recorded as wins).
2e. WHEEL IS DELIBERATELY OFF (wheel.enabled=false) — do not propose enabling it without working through the checklist in auto-memory. Its real track record is 0 wins / 7 losses: the 6 historical 'wins' in trades.log before 2026-07-27 are mislabeled losses from a fixed sign-inversion bug (-\$136.50 real), plus an orphaned position closed at -\$55 on 2026-08-21. Four blockers remain open: it has NO stop-loss of any kind (only exits are profit-close or assignment — structurally the same 'no floor' setup rejected for stocks on 08-20), flat 5% OTM strikes for every stock (the same defect fixed on the stock side), no assignment-risk sizing against available cash (cash is negative/margin), and no cap on concurrent wheels. Critically it also CANNOT be backtested — this account has no OPRA agreement so no real historical option prices exist at any horizon, and backtest/wheel_replay.py uses Black-Scholes estimates only. Any WHEEL result you produce is an estimate, never real-fill evidence; label it as such.
